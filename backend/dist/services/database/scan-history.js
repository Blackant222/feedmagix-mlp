...
export class ScanHistoryService extends BaseService {
    ...
    async getPetScans(petId, userId, options = {}) {
        return this.findAll({ user_id: userId, pet_id: petId }, {
            page: options.page,
            limit: options.limit,
            sort: 'scanned_at',
            order: 'desc'
    ...
    async updateScanResults(scanId, userId, results) {
        // Verify ownership
    ...
    async getUserScanStats(userId) {
      ...
      `)
        ...
        const brandCounts = scans.reduce((acc, scan) => {
    ...
    async getTrendingScans(limit = 10) {
      ...
      `)
        ...
        const productCounts = (data || []).reduce((acc, scan) => {
        ...
        return Object.values(productCounts).sort((a, b) => b.scan_count - a.scan_count);
    }
    // Delete user's scan history (GDPR compliance)
    async deleteUserScans(userId) {
        const { data, error } = await this.supabase
            .from(this.tableName)
            .delete()
            .eq('user_id', userId)
    ...
    async getScansByVerdict(userId, verdict, options = {}) {
        const { data, error } = await this.supabase
            .from(this.tableName)
            .select(`
        *,
