...
export async function chatRoutes(fastify) {
    ...
    fastify.get('/sessions', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
    ...
    fastify.post('/sessions', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
        const user = getAuthUser(request);
    ...
    fastify.get('/sessions/:id', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
        const user = getAuthUser(request);
    ...
    // Update chat session
    fastify.put('/sessions/:id', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
        const user = getAuthUser(request);
        const { id } = request.params;
    ...
    // Delete chat session
    fastify.delete('/sessions/:id', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
        const user = getAuthUser(request);
        const { id } = request.params;
        if (!validateUUID(id)) {
            throw new ValidationError('Invalid session ID format');
    ...
    fastify.get('/sessions/:id/messages', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
        const user = getAuthUser(request);
        const { id } = request.params;
    ...
    fastify.post('/sessions/:id/messages', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
        const user = getAuthUser(request);
        const { id } = request.params;
    ...
    fastify.get('/search', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
        ...
        try {
            ...
            const { data: messages, count } = await chatService.searchMessages(user.id, q.trim(), options);
            const response = {
                success: true,
                data: messages,
                pagination: {
    ...
    fastify.post('/sessions/:id/reset', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
        const user = getAuthUser(request);
        const { id } = request.params;
        if (!validateUUID(id)) {
            throw new ValidationError('Invalid session ID format');
    ...
    fastify.get('/sessions/:id/export', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
        const user = getAuthUser(request);
        const { id } = request.params;
        if (!validateUUID(id)) {
            throw new ValidationError('Invalid session ID format');
