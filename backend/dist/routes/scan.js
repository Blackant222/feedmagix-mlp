import { scanHistoryService, foodProductService, petService } from '../services/database/index.js';
import { runAIPipeline, processImage } from '../services/ai/ai-pipeline.js';
...
export async function scanRoutes(fastify) {
    ...
    fastify.post('/upload', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
    ...
    fastify.get('/history', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
        const user = getAuthUser(request);
        const { page = 1, limit = 10, pet_id, date_from, date_to, sort = 'scanned_at', order = 'desc' } = request.query;
        try {
            const options = {
                page: Number(page),
    ...
    fastify.get('/history/:id', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
        const user = getAuthUser(request);
    ...
    // Get trending scans
    fastify.get('/trending', asyncHandler(async (request, reply) => {
    ...
    fastify.get('/by-verdict', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
        const user = getAuthUser(request);
        ...
        try {
            ...
            const { data: scans, count } = await scanHistoryService.getScansByVerdict(user.id, verdict, options);
            const response = {
                success: true,
                data: scans,
                pagination: {
    ...
    fastify.delete('/history/:id', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
        const user = getAuthUser(request);
        const { id } = request.params;
        if (!validateUUID(id)) {
            throw new ValidationError('Invalid scan ID format');
    ...
    fastify.post('/quick-analyze', { preHandler: authMiddleware }, asyncHandler(async (request, reply) => {
        const user = getAuthUser(request);
