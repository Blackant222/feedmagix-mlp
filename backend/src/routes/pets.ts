...
export async function petRoutes(fastify: FastifyInstance) {
  ...
  // Create new pet
  fastify.post<{ Body: CreatePetRequest }>('/', 
    { preHandler: authMiddleware }, 
    asyncHandler(async (request, reply) => {
      const user = getAuthUser(request)!
  ...
  // Update pet
  fastify.put<{ Params: { id: string }, Body: UpdatePetRequest }>('/:id', 
    { preHandler: authMiddleware }, 
    asyncHandler(async (request, reply) => {
      const user = getAuthUser(request)!
      const { id } = request.params
  ...
  fastify.get<{ Params: { id: string }, Querystring: { limit?: number } }>('/:id/scans', 
    { preHandler: authMiddleware }, 
    asyncHandler(async (request, reply) => {
      const user = getAuthUser(request)!
      const { id } = request.params
  ...
  // Get pets by species
  fastify.get<{ Querystring: { species: string } }>('/by-species', 
    { preHandler: authMiddleware }, 
    asyncHandler(async (request, reply) => {
      const user = getAuthUser(request)!
