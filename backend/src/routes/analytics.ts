...
export async function analyticsRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: TrackEventRequest }>('/track', 
  ...
  fastify.get('/product-stats', 
    asyncHandler(async (request, reply) => {
      try {
        ...
        const categoryDistribution = (categoryData || []).reduce((acc: Record<string, number>, product) => {
        ...
        const categoryScores = (scoreData || []).reduce((acc: Record<string, { total: number, count: number }>, item) => {
        ...
        const averageScoresByCategory = Object.fromEntries(
