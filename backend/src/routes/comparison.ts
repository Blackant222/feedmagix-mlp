...
export async function comparisonRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: CreateComparisonRequest }>('/create', 
  ...
  fastify.post<{ Body: AnalyzeComparisonRequest }>('/analyze', 
    { preHandler: authMiddleware }, 
    asyncHandler(async (request, reply) => {
      const user = getAuthUser(request)!
      ...
      try {
        ...
        const comparisonData = await comparisonService.getComparisonWithProducts(
          comparison_id, 
          user.id
        )
        ...
        const productsForAnalysis = comparisonData.products
          ...
          .map(product => ({
            identification: {
              ...
              size: 'Unknown',
              food_type: product.category as any || 'kibble',
              species: comparisonData.pet?.species as any || 'dog'
            },
            nutrition_analysis: {
        ...
        const aiAnalysis = await getComparisonRecommendation(
          productsForAnalysis,
          {
        ...
        const updatedComparison = await comparisonService.updateComparisonResults(
          comparison_id,
          user.id,
          aiAnalysis
        )

        const response: AnalyzeComparisonResponse = {
          success: true,
          data: aiAnalysis,
          timestamp: new Date().toISOString()
        }
  ...
  fastify.get<{ Querystring: FilterQuery }>('/history', 
    ...
    asyncHandler(async (request, reply) => {
      ...
      try {
        ...
        const { data: comparisons, count } = await comparisonService.getUserComparisons(
          user.id, 
          options
        )

        const response: ComparisonHistoryResponse = {
          success: true,
          data: comparisons,
          pagination: {
            page: options.page,
  ...
  fastify.get<{ Querystring: { limit?: number } }>('/popular', 
    asyncHandler(async (request, reply) => {
  ...
  fastify.post<{ Body: { scan_ids: string[], pet_id: string } }>('/quick', 
    { preHandler: authMiddleware }, 
    asyncHandler(async (request, reply) => {
      const user = getAuthUser(request)!
  ...
  fastify.post<{ Body: { comparison_id: string, pet_id?: string } }>('/reanalyze', 
    { preHandler: authMiddleware }, 
    asyncHandler(async (request, reply) => {
      const user = getAuthUser(request)!
      ...
      try {
        ...
        if (!pet_id || pet_id === comparisonData.comparison.pet_id) {
          await comparisonService.updateComparisonResults(
            comparison_id,
            user.id,
            aiAnalysis
          )
