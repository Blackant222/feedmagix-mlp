...
export class FoodProductService extends BaseService {
  protected tableName = 'food_products'
  ...
  async updateProductDetails(productId: string, details: {
    ingredients?: string[]
  ...
  async findByBrand(brand: string, options: {
    page?: number
    limit?: number
  } = {}): Promise<{ data: FoodProduct[], count: number }> {
  ...
  async findBySpecies(species: string, options: {
    page?: number
    limit?: number
  } = {}): Promise<{ data: FoodProduct[], count: number }> {
  ...
  async getPopularProducts(limit: number = 10): Promise<Array<FoodProduct & { scan_count: number }>> {
    const { data, error } = await this.supabase
  ...
  async getTopRatedProducts(species?: string, limit: number = 10): Promise<Array<FoodProduct & { avg_score: number }>> {
    let queryBuilder = this.supabase
      .from('product_analytics')
      .select(`
  ...
  async updateProductAnalytics(productId: string, scanResult: {
    ...
    if (existing) {
      ...
      const newAvgScore = (
        (existing.avg_compatibility_score * existing.scan_count) + scanResult.compatibility_score
      ) / newScanCount
