...
export async function processImage(imageData: string, mimeType: string = 'image/jpeg'): Promise<ProcessedImage> {
  try {
...
export async function identifyProduct(imageData: string): Promise<ProductIdentification> {
  const prompt = `
...
`
  ...
  try {
    const result = await generateStructuredOutput<ProductIdentification>(
      prompt,
      schema,
...
export async function retrieveProductData(identification: ProductIdentification): Promise<ProductDataRetrieval> {
  const searchQuery = `
...
`
    ...
    const structuredData = await generateStructuredOutput<ProductDataRetrieval>(
      extractPrompt,
      schema,
      { temperature: 0.2 }
    )
...
export async function analyzeNutrition(
  ...
  productData: ProductDataRetrieval,
  petContext: {
    species: string
...
`
  ...
  try {
    const result = await generateStructuredOutput<NutritionAnalysis>(
      analysisPrompt,
      schema,
      { temperature: 0.5 }
    )
...
export async function createDelightOutput(
  nutritionAnalysis: NutritionAnalysis,
  petName?: string
): Promise<DelightOutput> {
  const delightPrompt = `
...
`
  ...
  try {
    const result = await generateStructuredOutput<DelightOutput>(
      delightPrompt,
      schema,
...
export async function runAIPipeline(input: AIPipelineInput): Promise<CompleteScanResult> {
  const startTime = Date.now()
  const results: AIPipelineStageResult[] = []
