...
export const Comparison: React.FC = () => {
  ...
  // Add mock products if comparison is empty
  const displayData = comparisonData.length > 0 ? comparisonData : mockProducts.map((product, index) => ({
    ...product,
    compatibilityScore: 85 - (index * 5),
    verdict: index === 0 ? 'بهترین انتخاب' : 'انتخاب خوب',
    pros: ['پروتئین بالا', 'مناسب سن'],
    cons: index > 0 ? ['قیمت بالا'] : []
  }));

  const winner = displayData.reduce((prev, current) => 
        ...
        <div className="px-6">
          ...
          <div className="space-y-4">
            {displayData.map((product, index) => (
              <Card 
                key={product.id} 
                className={`p-4 ${product.id === winner.id ? 'ring-2 ring-[#F59E0B] bg-orange-50/30' : ''}`}
              >
